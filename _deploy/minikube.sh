...
if [ -z "$ISO" ]; then
    ...
    if [ -x "$GH_BIN" ]; then
        ...
        ISO="minikube-amd64-${version}.iso"
...
echo ">> Starting Minikube (iso: ${ISO_REALPATH})"
MEM=${MEM:-8g}
CPU=${CPU:-4}
./minikube/out/minikube start --memory="${MEM}" --cpus="${CPU}" --iso-url="file://${ISO_REALPATH}" --cni=cilium --container-runtime=porto --cache-images=false
